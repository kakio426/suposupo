...
export function getRepairHint(question) {
  if (
    question.visualModel?.type === "carry-blocks" ||
    question.skillId === "two_digit_with_carry"
  ) {
    return "일의 자리에서 10개가 모이면 십의 자리로 조각 1개가 올라가요.";
  }
...
function createQuestionBySkill(skillId, level) {
  ...
  if (skillId === "two_digit_with_carry") {
    ...
    return {
      question: `${a} + ${b} = ?`,
      correctAnswer,
      visualModel: createCarryBlocksModel(a, b),
      options: buildOptions(correctAnswer, [
        correctAnswer - 10,
        correctAnswer - 1,
        correctAnswer + 1
...
function createPlacementQuestion(level, skillId, a, b, cOrPrompt, maybePrompt) {
  ...
  return {
    ...
    question,
    correctAnswer,
    visualModel: createPlacementVisualModel(skillId, a, b),
    options: buildOptions(correctAnswer, [
      correctAnswer - 1,
      correctAnswer + 1,
      correctAnswer - 10,
...
function createPlacementVisualModel(skillId, a, b) {
  if (skillId === "make_10") return createTenFrameModel(a, b);
  if (skillId === "two_digit_with_carry") return createCarryBlocksModel(a, b);
  return undefined;
}
...
function createCarryBlocksModel(a, b) {
  const leftOnes = a % 10;
  const rightOnes = b % 10;
  const onesSum = leftOnes + rightOnes;
  const carry = Math.floor(onesSum / 10);
  const remainder = onesSum % 10;
  const leftTens = Math.floor(a / 10);
  const rightTens = Math.floor(b / 10);
